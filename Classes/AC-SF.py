...
class ActorCritic(object):
    ...
    def _compute_policy_gradient(self, state, action, advantage):
        ...
        return grad.T, -(action - self.mean_policy.T.dot(state)) / (self.sigma ** 2) * advantage
    ...
    def update(self, state, action, reward, next_state, done, I):
        ...
        grad, _ = self._compute_policy_gradient(state, action, advantage)
    ...
    def estimator_variance(self, nr_samples):
        ...
        for i in range(nr_samples):
            ...
            next_state = policy._rbf_transform(next_state)
            advantage = reward + self.gamma * self._compute_value(next_state, done) - self._compute_value(state)
            _, grad = self._compute_policy_gradient(state, action, advantage)
...
def train(env, policy):
    ...
    plt.savefig('Results/' + "Moving Average RBF Actor Critic Score" + '.png')
...
for t in range(1000):
    ...
    if done:
        break
env.close()
save_frames_as_gif(frames, filename='AC-SF.gif')

visualize(plotting_rewards, ylabel="Reward", title="Reward Actor-Critic Score Function (RBF)")
visualize(plotting_variance, ylabel="Variance", title="Variance Actor-Critic Score Function (RBF)")